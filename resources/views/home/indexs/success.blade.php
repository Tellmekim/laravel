...
<section class="sec_score">
	<div class="container">
		<div class="score_box text-center" style="margin-top: 70px;">
		    @foreach($data as $k=> $v)
				@if ($v<=$score_info->score)
           		 <img src="/home/image/star{{$k+1}}_1.png" class="star{{$k+1}} wow animated zoomInDown animated" data-wow-delay="{{0.5+($k)/10}}s" style="visibility: visible; animation-delay:{{0.5+($k)/10}}s; animation-name: zoomInDown;">
				@else
				<img src="/home/image/star{{$k+1}}_0.png" class="star{{$k+1}} wow animated zoomInDown animated" data-wow-delay="{{0.5+($k)/10}}s" style="visibility: visible; animation-delay:{{0.5+($k)/10}}s; animation-name: zoomInDown;">
				@endif
			@endforeach
			...
			<p class="p2 wow animated bounceIn animated" data-wow-delay="1.4s" style="margin-bottom: 0px; visibility: visible; animation-delay: 1.4s; animation-name: bounceIn;">{{$score_info->score}}</p>
			...
			<p class="p3 clearfix wow animated bounceIn animated" data-wow-delay="1.6s" style="visibility: visible; animation-delay: 1.6s; animation-name: bounceIn;">
				<span class="left s1">Right :  {{$score_info->right}}</span>
				<span class="right s2">Wrong :   {{$score_info->wrong}}</span>
			</p>
